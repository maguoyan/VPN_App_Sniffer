...
def test_csv_to_group():
    # 测试数据：你可以使用一个小的测试文件或手动构造一个数据框来测试
    test_data = {
        'timestamp': [0.00001, 0.00002, 0.00003, 0.00004, 0.00005],

        'length': [10, 20, 15, 5, 30],
        'direction': [1, 1, -1, -1, 1]
    }

    df_test = pd.DataFrame(test_data)
    df_test.to_csv('test1.csv', index=False)  # 将测试数据保存为 CSV 文件
    processed_data = csv_to_group('test1.csv')
